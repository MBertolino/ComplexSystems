    ...
    for t = 2:T
        for n = 1:N
            ...
            if (chance < p)
                ...
                dir_new = pi + atan((neigh_y - status(n, 2))/(neigh_x - status(n, 1)));
                theta_new(n) = wrapTo2Pi(dir_new);
            end
            ...
            if (chance < q(iq) && chance > p)
                ...
                theta_new(n) = status(follow_me, 3);
            end
            % Add noise
        ...
        psi(t, iq) = (sum(cos(status(:, 3))).^2 + sum(sin(status(:, 3))).^2)/N;
